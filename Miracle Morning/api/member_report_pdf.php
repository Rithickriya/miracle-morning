...
<div class="report-wrap">
    ...
    <table class="data-table">
        <thead><tr><th>Paid Date</th><th>Weeks</th><th>Sundays Covered</th><th>Mode</th><th>Status</th><th>Total</th></tr></thead>
        <tbody>
            ...
            <tr>
                ...
                <td class="center" style="color:<?= $s['status']==='Paid'?'#1b5e20':($s['status']==='Pending'?'#c47800':'#c62828') ?>;font-weight:bold"><?= htmlspecialchars($s['status']) ?></td>
    ...
    <?php if ($byMonth): ?>
        ...
        ?>
            ...
            <table class="data-table">
                <thead><tr><th>Sunday Date</th><th>Status</th><th>Mode</th><th>Amount</th><th>Paid On</th></tr></thead>
                <tbody>
                    <?php foreach ($mRows as $r):
                        ...
                        elseif ($r['status'] === 'Pending') { $ss = ($r['friday_date'] < $today) ? 'Due' : 'Pending'; }
                        else { $ss = $r['status']; }
                    ?>
                        <tr>
    ...
    <?php if ($kittyTxns): ?>
        ...
        <table class="summary-table">
            <tr>
                <td class="right">Total paid</td><td class="right bold">₹<?= number_format($kittyPaid) ?></td>
                <td class="right">Balance</td><td class="right bold"><?= $kittyBal > 0 ? '₹' . number_format($kittyBal) : 'Fully Paid' ?></td>
            </tr>
        </table>
    <?php else: ?>
    ...
    <?php if ($visPending > 0 || $visDue > 0): ?>
        <div class="note-box">
            <?php if ($visPending > 0): ?><span class="bold orange"><?= $visPending ?> visitor entr<?= $visPending > 1 ? 'ies' : 'y' ?> pending — ₹<?= number_format($visPendingTot) ?></span><?php endif; ?>
            <?php if ($visPending > 0 && $visDue > 0): ?><br><?php endif; ?>
            <?php if ($visDue > 0): ?><span class="bold red"><?= $visDue ?> visitor fee<?= $visDue > 1 ? 's' : '' ?> due from member — ₹<?= number_format($visDueTot) ?></span><?php endif; ?>
        </div>
    <?php endif; ?>
    <?php if ($visitors): ?>
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="bold"><?= htmlspecialchars($v['visitor_name']) ?></span><?php if (!empty($v['visitor_company'])): ?><br><span class="small"><?= htmlspecialchars($v['visitor_company']) ?></span><?php endif; ?></td>
        ...
        <table class="summary-table">
            <tr>
                <td class="right">Total visitors</td><td class="center bold"><?= count($visitors) ?></td>
                <td class="right">Collected</td><td class="right bold">₹<?= number_format($visTot) ?></td>
                <?php if ($visPending > 0): ?><td class="right orange">Pending</td><td class="right bold orange">₹<?= number_format($visPendingTot) ?></td><?php endif; ?>
                <?php if ($visDue > 0): ?><td class="right red">Due from member</td><td class="right bold red">₹<?= number_format($visDueTot) ?></td><?php endif; ?>
            </tr>
        </table>
    <?php else: ?>
